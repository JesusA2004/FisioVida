...
class RolesController extends Controller {
    ...
    public function store(RoleStoreRequest $request) {
        DB::transaction(function () use ($request) {
            ...
            app(AuditLogService::class)->created(
                ...
                'role',
                $role->id,
                'El usuario '.$request->user()?->name.' creó el rol '.$role->name.'.',
                ['permissions' => $permissionIds] + $data
            );
        });
    ...
    public function update(RoleUpdateRequest $request, Role $role) {
        DB::transaction(function () use ($request, $role) {
            ...
            app(AuditLogService::class)->updated(
                ...
                'role',
                $role->id,
                'El usuario '.$request->user()?->name.' actualizó el rol '.$role->name.'.',
                $before,
                ['permissions' => $permissionIds] + $data
            );
        });
    ...
    public function destroy(Role $role) {
        ...
        app(AuditLogService::class)->deleted(
            ...
            'role',
            $role->id,
            'El usuario '.request()->user()?->name.' eliminó el rol '.$role->name.'.',
            $before
        );
