...
class LogsController extends Controller {
    ...
    public function index(Request $request) {
        ...
        if ($module !== '') $query->where(DB::raw('COALESCE(l.module, l.entity_type)'), $module);
        if ($action !== '') $query->where('l.action', $action);
        if ($userId > 0) $query->where(DB::raw('COALESCE(l.user_id, l.actor_user_id)'), $userId);
        if ($start !== '') $query->whereDate('l.created_at', '>=', $start);
        if ($end !== '') $query->whereDate('l.created_at', '<=', $end);
        ...
        return Inertia::render('Logs/Index', [
            ...
            'filters' => $this->filters($request, ['q', 'module', 'action', 'user_id', 'start_date', 'end_date']),
            'lookups' => [
                'users' => DB::table('users')->whereNull('deleted_at')->orderBy('name')->limit(200)->get(['id', DB::raw('name as label')]),
                'modules' => DB::table('logs')->select(DB::raw('COALESCE(module, entity_type) as value'))->whereNotNull(DB::raw('COALESCE(module, entity_type)'))->distinct()->orderBy('value')->pluck('value'),
                'actions' => DB::table('logs')->select('action')->whereNotNull('action')->distinct()->orderBy('action')->pluck('action'),
            ],
        ]);
    }
