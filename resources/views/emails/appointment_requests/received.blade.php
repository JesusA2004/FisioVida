...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f7f8;padding:32px 16px;">
  <tr><td align="center">
    <table width="100%" cellpadding="0" cellspacing="0" style="max-width:640px;background:#ffffff;border-radius:24px;overflow:hidden;border:1px solid #e5e7eb;box-shadow:0 14px 40px rgba(15,23,42,0.08);">
      <tr>
        <td style="padding:30px;background:linear-gradient(135deg,#7c3aed,#0EA5A4);color:#ffffff;">
          <h1 style="margin:0;font-size:26px;font-weight:700;">📅 Solicitud recibida</h1>
          <p style="margin:10px 0 0;font-size:15px;">Recibimos tu solicitud de cita en FisioVida.</p>
        </td>
      </tr>
      <tr><td style="padding:30px;">
        <p style="margin:0 0 16px;font-size:15px;color:#4b5563;">Hola <strong>{{ $patient_name }}</strong>,</p>
        <p style="margin:0 0 22px;font-size:15px;color:#4b5563;">Tu solicitud de cita fue recibida correctamente. Nuestro equipo la revisará pronto y te confirmaremos tu cita.</p>
        <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc;border:1px solid #e5e7eb;border-radius:18px;margin:0 0 24px;overflow:hidden;">
          ...
          <tr><td style="padding:16px 20px;border-bottom:1px solid #e5e7eb;">
            <p style="margin:0 0 4px;font-size:12px;color:#6b7280;text-transform:uppercase;">Fecha preferida</p>
            <p style="margin:0;font-size:16px;font-weight:700;color:#111827;">{{ $date_label }}{{ $preferred_time ? ' — '.$preferred_time : '' }}</p>
          </td></tr>
          @endif
          @if($reason)
          <tr><td style="padding:16px 20px;">
            <p style="margin:0 0 4px;font-size:12px;color:#6b7280;text-transform:uppercase;">Motivo</p>
            <p style="margin:0;font-size:14px;color:#374151;">{{ $reason }}</p>
          </td></tr>
          @endif
        </table>
        <div style="text-align:center;margin:24px 0;">
          <a href="{{ $login_url }}" style="display:inline-block;background:#0EA5A4;color:#fff;text-decoration:none;padding:14px 26px;border-radius:16px;font-weight:700;font-size:15px;">Ver mi portal</a>
        </div>
      </td></tr>
