...
export const useCitaCrud = (filters: { q?: string; status?: string }) => {
    ...
    const submit = async () => {
        const ok = await swalConfirm(
            ...
            'Se actualizará la agenda del paciente y terapeuta.',
        ...
        const options = {
            ...
            onSuccess: () => {
                swalToast(
                    editingId.value ? 'Cita actualizada' : 'Cita creada',
                    'success',
                );
                closeModal();
            },
    ...
    const markNoShow = async (row: CitaRow) => {
        ...
        router.put(
            ...
            },
            {
                ...
                onError: () => swalErr('No se pudo actualizar la cita'),
            },
        );
    };
    ...
    const advanceStatus = async (row: CitaRow) => {
        ...
        if (!next) return;
        const ok = await swalConfirm(
            '¿Avanzar estado de cita?',
            `${row.patient_name} → ${next}`,
            'Sí, avanzar',
        );
        if (!ok) return;
        router.put(
