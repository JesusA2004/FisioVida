...
const ziggyRoute = (name: string, params?: any) => {
    const fn = (window as any).route as
        | undefined
        | ((n: string, p?: any) => string);
...
const hasStatus = computed(() =>
    props.fields.some((f) => String(f.key) === 'status'),
);
...
const submit = () => {
    if (!hasStore.value) {
        swalErr(
            ...
            'Define routeBase o routes.* para este CRUD.',
        );
        return;
    }
    ...
    if (editingRow.value) {
        if (!hasUpdate.value) {
            swalErr(
                'Ruta update faltante',
                'Configura routes.update o routeBase.update.',
            );
            return;
        }
        const u = requireUrl(
            routeName.value.update,
            getRouteParam(editingRow.value),
        );
        if (!u) return;
...
<template>
    <FvPageShell>
        <div class="w-full max-w-none">
            ...
            >
                <div class="min-w-0">
                    <div class="flex items-center gap-2">
                        ...
                        <span
                            v-if="activeFilters"
                            class="inline-flex items-center gap-2 rounded-full bg-accent/40 px-3 py-1 text-xs font-medium text-accent-foreground ring-1 ring-border/40 dark:bg-accent/25"
                        >
                    ...
                    <div class="mt-2 text-xs text-muted-foreground">
                        ...
                        <span v-if="page?.total" class="mx-2 opacity-50"
                            >·</span
                        >
                        <span v-if="page?.total"
                            >Mostrando {{ page.from ?? 0 }}–{{
                                page.to ?? 0
                            }}</span
                        >
                    </div>
            ...
            <div
                class="relative mt-5 w-full overflow-hidden rounded-3xl border border-border/50 shadow-sm"
            >
                <!-- Fondo premium light/dark -->
                <div
                    aria-hidden="true"
                    class="absolute inset-0 bg-gradient-to-br from-white via-white to-accent/20 dark:from-card dark:via-card dark:to-accent/10"
                />
                <div
                    aria-hidden="true"
                    class="absolute -top-24 -right-24 h-72 w-72 rounded-full bg-primary/20 opacity-40 blur-3xl dark:bg-primary/10"
                />
                <div class="relative p-4 sm:p-5 2xl:p-6">
                    ...
                    >
                        <div
                            class="flex w-full flex-col gap-3 sm:flex-row sm:flex-wrap xl:w-auto"
                        >
                            <div
                                v-if="searchable"
                                class="w-full sm:w-[380px] 2xl:w-[520px]"
                            >
                                ...
                                <div class="relative mt-1">
                                    ...
                                    <button
                                        v-if="local.q"
                                        class="absolute top-1/2 right-2 grid h-9 w-9 -translate-y-1/2 place-items-center rounded-full transition hover:bg-accent/50"
                                        type="button"
                                        @click="local.q = ''"
                            ...
                            <div
                                v-if="hasStatus && statusOptions?.length"
                                class="w-full sm:w-[240px] 2xl:w-[280px]"
                            >
                                <Label class="text-xs text-muted-foreground"
                                    >Estatus</Label
                                >
                                <SearchableSelect
                                    :model-value="local.status || null"
                                    :options="[
                        ...
                        <!-- Pager -->
                        <div
                            class="flex items-center justify-between gap-2 xl:justify-end"
                        >
                            <Button
                                variant="secondary"
                                class="rounded-full shadow-sm"
                    ...
                    <div class="mt-4">
                        ...
                        <div class="grid gap-3 lg:hidden">
                            <div
                                v-for="row in rows"
                                :key="getRowKey(row)"
                                class="rounded-3xl border border-border/50 bg-white/70 p-4 backdrop-blur-sm transition hover:bg-accent/20 dark:bg-background/60"
                            >
                                <div class="grid gap-2">
                                    <div
                                        v-for="c in columns.filter(
                                            (x) => !x.hideOnMobile,
                                        )"
                                        :key="c.label"
                                    ...
                                    >
                                        ...
                                        <div
                                            class="max-w-[60%] text-right text-sm break-words"
                                        >
                                            {{
                                                c.mobileValue
                                                    ? c.mobileValue(row)
                                ...
                                <div class="mt-3 flex justify-end gap-2">
                                    <Button
                                        v-if="canEdit"
                                        variant="secondary"
                                        class="h-9 gap-2 rounded-full px-3 shadow-sm"
                                        @click="openEdit(row)"
                                    >
                            ...
                            <div
                                v-if="!rows.length"
                                class="rounded-3xl border border-dashed border-border/60 bg-white/70 p-10 text-center backdrop-blur-sm dark:bg-background/60"
                            >
                                <div class="text-sm font-medium">
                                    No hay registros
                                </div>
                                <div class="mt-1 text-sm text-muted-foreground">
                                    Crea el primero y deja la operación lista.
                                </div>
                                <div class="mt-4">
                                    <Button
                                        v-if="canCreate"
                        ...
                        <!-- Desktop table -->
                        <div
                            class="fv-no-scrollbar hidden overflow-auto rounded-3xl border border-border/40 bg-white/70 backdrop-blur-sm lg:block dark:bg-background/50"
                        >
                            <table class="w-full text-sm">
                                <thead
                                    class="sticky top-0 z-10 bg-white/90 text-xs text-muted-foreground backdrop-blur dark:bg-card/80"
                                >
                                    <tr class="border-b border-border/30">
                                        ...
                                        <th
                                            class="w-[210px] px-4 py-3 text-right font-medium"
                                        >
                                            Acciones
                                        </th>
                                    </tr>
                                ...
                                >
                                    <tr
                                        v-for="row in rows"
                                        :key="getRowKey(row)"
                                        class="transition hover:bg-accent/15"
                                    >
                                        <td
                                            v-for="c in columns"
                                            :key="c.label"
            ...
            <Dialog v-model:open="dialogOpen">
                <DialogContent
                    class="w-[calc(100vw-2rem)] overflow-hidden rounded-3xl border border-border/50 p-0 shadow-2xl sm:w-full"
                    :class="modalMax"
                >
                    <DialogHeader
                        class="border-b border-border/20 bg-white/90 p-5 backdrop-blur dark:bg-card/90"
                    >
                    ...
                    >
                        ...
                        >
                            ...
                            <textarea
                                ...
                                v-model="(form as any)[f.key]"
                                class="mt-1 min-h-[120px] w-full rounded-xl bg-white/70 px-3 py-2 text-sm ring-1 ring-border/40 transition outline-none focus:ring-2 focus:ring-primary/60 dark:bg-background"
                                :placeholder="f.placeholder"
                                :disabled="Boolean(f.readonly && editingRow)"
                            />
                                ...
                                <Popover>
                                    <PopoverTrigger as-child>
                                        <Button
                                            type="button"
                                            variant="secondary"
                                            class="h-11 w-full justify-start rounded-xl bg-white/70 shadow-sm dark:bg-background"
                                            :disabled="
                                                Boolean(
                                                    f.readonly && editingRow,
                                                )
                                            "
                                        >
                                            <CalendarIcon
                            ...
                            >
                                <Checkbox
                                    ...
                                    @update:checked="
                                        (v: any) => ((form as any)[f.key] = v)
                                    "
                                    :disabled="
                                        Boolean(f.readonly && editingRow)
                                    "
                                />
                                <span class="text-sm">{{
                                    (f as any).toggleLabel ?? 'Activo'
                                }}</span>
                            </div>

                            <SearchableSelect
                                v-else-if="f.type === 'select'"
                                :model-value="(form as any)[f.key] ?? null"
                                :options="[
                            ...
                            <input
                                ...
                                type="file"
                                class="mt-2 block w-full text-sm text-muted-foreground transition file:mr-3 file:rounded-full file:border-0 file:bg-accent/40 file:px-4 file:py-2 file:text-sm file:font-medium hover:file:bg-accent/60"
                                :accept="f.accept"
                                @change="
                                    (e: any) => {
                                        (form as any)[f.key] =
                    ...
                    <DialogFooter
                        class="border-t border-border/20 bg-white/90 p-5 backdrop-blur dark:bg-card/90"
                    >
