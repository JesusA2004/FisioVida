...
const moduleEnabled = (module: string) => enabledModules.value[module] !== false
const can = (permission: string) => isSuperAdmin.value || permissions.value.includes(permission)
const canAccess = (module: string, permission: string) => moduleEnabled(module) && can(permission)

const mainNavItems = computed<NavItem[]>(() => {
  const items: NavItem[] = []

  if (canAccess('dashboard', 'dashboard.view')) items.push({ title: 'Dashboard', href: '/dashboard', icon: LayoutGrid })
  if (canAccess('agenda', 'appointments.view')) items.push({ title: 'Agenda', href: '/citas', icon: CalendarDays })
  if (canAccess('pacientes', 'patients.view')) items.push({ title: 'Pacientes', href: '/pacientes', icon: Users })
  if (canAccess('sesiones', 'sessions.view')) items.push({ title: 'Sesiones', href: '/sesiones', icon: ClipboardList })
  ...
  if (canAccess('usuarios', 'users.view')) items.push({ title: 'Usuarios', href: '/usuarios', icon: Users })
  if (canAccess('logs', 'logs.view')) items.push({ title: 'Logs', href: '/logs', icon: BarChart3 })
  if (canAccess('configuracion', 'settings.view')) items.push({ title: 'Configuración', href: '/configuracion', icon: Settings })
