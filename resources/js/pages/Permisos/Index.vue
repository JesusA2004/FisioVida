...
const applyFilters = (extra: Record<string, string | number>) => {
  router.get(route('permisos.index'), { ...props.filters, ...extra }, { preserveState: true, replace: true })
}
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <section class="space-y-6 rounded-3xl bg-white p-6 shadow-xl transition-all duration-300 dark:bg-zinc-950">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold text-zinc-900 dark:text-zinc-100">Permisos por acción</h1>
          <p class="text-sm text-zinc-500 dark:text-zinc-400">Administra los permisos que pueden asignarse a cada rol.</p>
        </div>
      ...
      <div class="rounded-2xl border border-zinc-200 bg-zinc-50/60 p-4 dark:border-zinc-800 dark:bg-zinc-900/40">
        <div class="grid gap-3 md:grid-cols-3">
          <div class="relative md:col-span-2">
            ...
            <Input class="pl-9" :default-value="props.filters.q ?? ''" placeholder="Buscar por nombre o slug" @change="(e) => applyFilters({ q: (e.target as HTMLInputElement).value, page: 1 })" />
          </div>
          <select class="h-10 rounded-xl border border-zinc-200 bg-white px-3 text-sm dark:border-zinc-800 dark:bg-zinc-900" :value="props.filters.module ?? ''" @change="(e) => applyFilters({ module: (e.target as HTMLSelectElement).value, page: 1 })">
            <option value="">Todos los módulos</option>
            <option v-for="item in props.moduleOptions" :key="item" :value="item">{{ item }}</option>
          </select>
          <select class="h-10 rounded-xl border border-zinc-200 bg-white px-3 text-sm dark:border-zinc-800 dark:bg-zinc-900 md:col-span-1" :value="props.filters.status ?? ''" @change="(e) => applyFilters({ status: (e.target as HTMLSelectElement).value, page: 1 })">
      ...
      <div v-if="props.rows.length === 0" class="rounded-3xl border border-dashed border-zinc-300 bg-zinc-50 p-10 text-center dark:border-zinc-700 dark:bg-zinc-900/40">
        <h3 class="text-lg font-semibold text-zinc-800 dark:text-zinc-100">Sin permisos para mostrar</h3>
        <p class="mt-2 text-sm text-zinc-500 dark:text-zinc-400">Intenta ajustar los filtros o crea un nuevo permiso.</p>
      </div>
          ...
          <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <div class="flex items-center gap-2">
                ...
                <Badge class="rounded-full px-3 py-1 text-xs" :class="statusVariant(row.status)">{{ row.status }}</Badge>
              </div>
              <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{{ row.slug }}</p>
              <p class="mt-2 inline-flex rounded-full bg-sky-100 px-2 py-1 text-xs font-medium text-sky-700 dark:bg-sky-900/30 dark:text-sky-300">Módulo: {{ row.module }}</p>
            </div>
            <div class="flex flex-wrap gap-2">
              <Button variant="outline" class="rounded-xl" @click="openEdit(row)"><Pencil class="mr-2 h-4 w-4" />Editar</Button>
              <Button variant="outline" class="rounded-xl" @click="toggleStatus(row)"><Power class="mr-2 h-4 w-4" />{{ row.status === 'active' ? 'Desactivar' : 'Activar' }}</Button>
              <Button variant="destructive" class="rounded-xl" @click="destroyPermission(row.id)"><Trash2 class="mr-2 h-4 w-4" />Eliminar</Button>
            </div>
          </div>
        </article>
      </div>
