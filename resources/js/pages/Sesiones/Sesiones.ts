...
export function useSesionesCrud(props: SesionesPageProps) {
    const patients: SelectOpt[] = (props.lookups?.patients ?? []).map((p) => ({
        value: p.id,
        label: p.label,
    }));
    const therapists: SelectOpt[] = (props.lookups?.therapists ?? []).map(
        (t) => ({ value: t.id, label: t.label }),
    );
    const appointments: SelectOpt[] = (props.lookups?.appointments ?? []).map(
        (a) => ({ value: a.id, label: a.label }),
    );
    ...
    const columns: CrudColumn<SessionRow>[] = [
        {
            label: 'Fecha',
            class: 'min-w-[140px]',
            value: (r) => formatDateMx(r.session_date),
        },
        {
            label: 'Paciente',
            class: 'min-w-[240px] font-medium',
            value: (r) => r.patient_name,
        },
        {
            label: 'Terapeuta',
            class: 'min-w-[180px]',
            value: (r) => r.therapist_name,
        },
        {
            label: 'Dolor',
            class: 'min-w-[90px]',
            value: (r) => (r.pain_scale ?? '—').toString(),
        },
    ];
    ...
    const fields: CrudField<SessionForm>[] = [
        ...
        { key: 'subjective', label: 'Subjetivo', type: 'textarea' },
        { key: 'objective', label: 'Objetivo', type: 'textarea' },
        { key: 'assessment', label: 'Evaluación', type: 'textarea' },
