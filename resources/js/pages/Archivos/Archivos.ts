...
export function useArchivosCrud(props: ArchivosPageProps) {
    ...
    const sessions: SelectOpt[] = (props.lookups?.sessions ?? []).map((s) => ({
        value: s.id,
        label: s.label,
    }));
    ...
    const columns: CrudColumn<FileRow>[] = [
        {
            label: 'Fecha',
            class: 'min-w-[160px]',
            value: (r) => formatDateTimeMx(r.created_at),
        },
        {
            label: 'Paciente',
            class: 'min-w-[240px]',
        ...
        },
        {
            label: 'Sesión',
            class: 'min-w-[140px]',
            value: (r) => r.session_date ?? '—',
        },
        {
            label: 'Archivo',
            class: 'min-w-[260px] font-medium',
            value: (r) => r.original_name,
        },
        {
            label: 'Tamaño',
            class: 'min-w-[120px]',
            value: (r) => fmtSize(r.size_bytes),
        },
    ];
