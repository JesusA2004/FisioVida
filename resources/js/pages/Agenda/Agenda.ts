...
export function useAgendaCrud(props: AgendaPageProps) {
    const patientOpts: SelectOpt[] = (props.lookups?.patients ?? []).map(
        (p) => ({ value: p.id, label: p.label }),
    );
    const therapistOpts: SelectOpt[] = (props.lookups?.therapists ?? []).map(
    ...
    const columns: CrudColumn<AppointmentRow>[] = [
        {
            ...
            class: 'min-w-[210px]',
            value: (r) =>
                `${formatDateTimeMx(r.start_at)} → ${formatDateTimeMx(r.end_at)}`,
        },
        {
            label: 'Paciente',
            class: 'min-w-[220px]',
